...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        ### START CODE HERE (~ 8 Lines)
        # Compute tensor `X` of source sentences with shape (src_len, b, e) using the source model embeddings.
        X = self.model_embeddings.source(source_padded)
        
        # Apply the encoder, pack_padded_sequence, and pad_packed_sequence
        ...
        # Concatenate the forwards and backwards tensors to obtain init_decoder_hidden and init_decoder_cell.
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for Y_t in torch.split(Y, 1, dim=0):
            Y_t = Y_t.squeeze(0)  # Squeeze Y_t into a tensor of dimension (b, e)
            Ybar_t = torch.cat((Y_t, o_prev), dim=1)  # Concatenate Y_t with o_prev

            # Use the step function to compute the next (cell, state) values and the new combined output o_t.
        ...
        # Stack combined_outputs to get a single tensor shape (tgt_len, b, h)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        dec_hidden = dec_hidden.unsqueeze(2)  # (b, 1, h)
        e_t = torch.bmm(enc_hiddens_proj, dec_hidden).squeeze(2)  # (b, src_len)
        ...
        ### 5. Use batched matrix multiplication between alpha_t and enc_hiddens to obtain the
        ###    attention output vector, a_t.
        a_t = torch.bmm(alpha_t.unsqueeze(1), enc_hiddens).squeeze(1)  # (b, 2h)

        ### 6. Concatenate dec_hidden with a_t to compute tensor U_t
        U_t = torch.cat(( a_t, dec_hidden.squeeze(2)), dim=1)  # (b, 3h)
